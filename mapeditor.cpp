...
void MapEditor::paintEvent(QPaintEvent*)//绘图函数
{
    ...
    painter.drawPixmap(10, 140, 1080, 600, QPixmap(pic));//载入背景图片并显示出来
    QFont font1("Microsoft YaHei", 9);//字体说明
    QFont font2("Microsoft YaHei", 12);
    painter.drawRect(10, 140, 1080, 600);//矩形大小，窗口大小
    painter.setFont(font1);
    painter.setRenderHint(QPainter::Antialiasing, true);//使接下来的绘图光滑
    for (int i = 1; i <= num1; i++)
    {
        if (!line[i].f) painter.setPen(Qt::blue);//设置画笔颜色为蓝色：非最短路径
        else painter.setPen(Qt::red);//最短路径设置为红线
    ...
    for (int i = 1; i <= num2; i++)
    {
        painter.drawEllipse(P[i], 10, 10);//把点画出来
        painter.drawText(QPoint(P[i].x() + 10, P[i].y() + 10), Pl[i]);//画出点的标签，pl[i]为标签
    }
